/*
  Purpose: Run an experiment to determine the transfer function for the rotational velocity of the system. 
...
double Va2 = -3.5;
double deltaVa = 0;
double rotation_velocity_RadpS = 0;

// Va = Va1 + Va2 = 0 (not testing)
// deltaVa = Va1 - Va2 = 1 (under test) testing rotation
...
void loop() {
  ...
  if ((current_time >= 1) && (current_time <= 3 )) {
    ...
    Serial.print("\t");
    Serial.println(rotation_velocity_RadpS);
  }
  else {
    for (i=0; i<2; i++) {
